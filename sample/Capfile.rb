...
task :nc, :hosts => now_ip_addr do
	run 'mkdir -p /root/creds || echo "do nothing"'
	upload "/root/data/mycreds.zip","/root/creds/mycreds.zip"
...
task :nc, :hosts => now_ip_addr do
	...
	run '/etc/init.d/eucalyptus-nc restart || echo "do nothing"' 
	run '/etc/init.d/eucalyptus-cloud restart || echo "do nothing"' 
	run 'ntp ntp.tohtech.ac.jp || ntpdate ntp.tohtech.ac.jp || echo "do nothing"'
	run 'yes | aptitude safe-upgrade || yum update -y'
	run 'yes | cp /usr/share/zoneinfo/Japan /etc/localtime || echo "do nothing"'
	run 'yes | passwd eucalyptus'
end
